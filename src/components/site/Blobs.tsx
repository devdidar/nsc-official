export function Blobs({ variant = "default" }: { variant?: "default" | "soft" }) {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle at 60% 40%, oklch(0.86 0.16 205 / 0.55), transparent 60%)", animationDelay: "-6s" }}
      />
      {variant === "default" && (
        <div
          className="absolute -bottom-40 left-1/3 h-[500px] w-[500px] rounded-full opacity-25 blur-3xl animate-float-slow"
          style={{ background: "radial-gradient(circle at 50% 50%, oklch(0.55 0.22 280 / 0.5), transparent 60%)", animationDelay: "-10s" }}
        />
      )}
    </div>
  );
