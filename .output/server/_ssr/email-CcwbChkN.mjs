...
//#region node_modules/.nitro/vite/services/ssr/assets/email-CcwbChkN.js
...
var sendContactEmail = createServerFn({ method: "POST" }).validator((data) => data).handler(sendContactEmail_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (response.error) {
			...
			return {
				success: false,
				error: `Email failed: ${response.error.message || "Unknown error"}`
			};
		}
		const { error: submitError } = await supabase.from("contact_messages").insert([data]);
		if (submitError) {
